<script lang="ts">
  ...
  export let monthLimit = 0;
  export let monthRemaining = 0;
  export let monthReset = 0;
  export let lastUsed = 0;
  ...
  function FriendlyTimeAgo (timestamp: number): string {
...
{#if mode === 'view'}
  <div class="list-group-item list-group-item-action">
    <div class="d-flex w-100 justify-content-between">
      ...
      <small>{FriendlyTimeAgo(lastUsed)}</small>
    </div>
    <p class="mb-1">
      ...
      <div class="col">
        <div class="mb-3">
          ...
          <input
            ...
            class="form-control"
            id="key-name"
            placeholder="Name (e.g., 'Search Pro')"
            bind:value="{name}" />
        </div>
        <div class="mb-3">
          ...
          <input
            ...
            class="form-control"
            id="key-value"
            placeholder="Key (e.g., 'BSAIsT3x-fl3B3kKmRONwKc6JTqgxFq')"
            bind:value="{key}" />
        </div>
        <div>
          ...
          {#each Endpoints as endpoint}
            <div class="form-check">
              <input
                ...
                type="checkbox"
                checked="{associatedEndpoints.includes(endpoint.value)}"
                id="endpoint-{endpoint.value}"
                on:change="{updateEndpoint}" />
