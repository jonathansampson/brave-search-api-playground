<script lang="ts">
    ...
    let safesearch: string = 'moderate';
    let freshness: string = '';
    let goggles_id: string = '';
    let units: string = '';
    let count: number = 20;
    let offset: number = 0;
    ...
    export const getParameters = () => {
        ...
        params = {
            ...
            search_lang, count, offset, safesearch,
            text_decorations: checkboxes[0].checked,
            spellcheck: checkboxes[1].checked,
            result_filter: result_filters.filter( f => f.checked ).map( f => f.id ).join(','),
...
<div class="mb-3 row">
    ...
    <div class="col-6">
        ...
        (<a class="link-secondary" target="_blank" href={"https://raw.githubusercontent.com/brave/goggles-quickstart/main/goggles/tech_blogs.goggle"}>Example</a>)
        <input bind:value={goggles_id} type="text" class="form-control" id="goggles_id" placeholder={"raw.githubusercontent.com/brave/goggles-quickstart/main/goggles/tech_blogs.goggle"}>
    </div>
</div>
    ...
    <div class="col-6">
        ...
        {#each result_filters.slice(0, Math.ceil(result_filters.length / 2)) as { id, label, checked }}
            <div class="form-check form-switch">
                <input class="form-check-input" type="checkbox" role="switch" id={id} bind:checked={checked} />
